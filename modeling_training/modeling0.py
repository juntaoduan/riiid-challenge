...
# https://github.com/tensorflow/addons/blob/v0.12.0/tensorflow_addons/layers/multihead_attention.py#L23-L298
class HocMultiHeadAttention(tfa.layers.MultiHeadAttention):
    ...
    def build(self, input_shape):
        ...
        self.query_kernel_mod = self.add_weight(
            name="query_kernel_mod",
            shape=[self.num_heads, num_query_features, num_key_features],
    ...
    def call(self, inputs, training=None, mask=None, attn_weight = None):
        ...
        #print('query_kernel ', self.query_kernel.shape)
        # there is no need to transform key, we insert a square matrix between query and key
        # Scale dot-product,
        depth = tf.constant(self.head_size, dtype=tf.float32)
        query /= tf.sqrt(depth)

        # take square higher order
        #query = tf.math.square(query)
        key = tf.math.square(key)
        value = tf.math.square(value)

        # key MI -> MIH  (create H copies) 
        dimen_key_addhead = [1]*len(key.shape)+[self.num_heads]
        key = tf.expand_dims(key, -1)
        key = tf.tile(key, dimen_key_addhead)
        # swap the head to second last axis MIH -> MHI
        key = tf.einsum("...ij->...ji", key)

        
        #S= # key_features = I
        query = tf.einsum("...NI , HIS -> ...NHS", query, self.query_kernel_mod)
        #key1 = tf.einsum("...MI , HIO -> ...MHO", key_squre, self.key_kernel)
        value = tf.einsum("...MI , HIO -> ...MHO", value, self.value_kernel)
        
        # Linear transformation
        #query2 = tf.einsum("...NI , HIO -> ...NHO", query, self.query_kernel)
        #key2 = tf.einsum("...MI , HIO -> ...MHO", key, self.key_kernel)
  
        

        # Calculate dot product attention
        logits = tf.einsum("...NHI,...MHI->...HNM", query, key)
        #logits2 = tf.einsum("...NHI,...MHI->...HNM", query2, key2)
        # adding second order info
        # logits = logits + tf.math.square(logits)
        # apply mask
        if mask is not None:
...
class WeightedMultiHeadAttention(tfa.layers.MultiHeadAttention):
    ...
    def build(self, input_shape):
        ...
        self.key_kernel_mod = self.add_weight(
            name="key_kernel_mod",
            shape=[self.num_heads, num_key_features, num_key_features],
            initializer=self.kernel_initializer,
            regularizer=self.kernel_regularizer,
            constraint=self.kernel_constraint,
        )
    ...
    def call(self, inputs, training=None, mask=None, attn_weight = None):
        ...
        # ------------------------
        # N = query elements
        # M = number of key/value elements
        # H = head size
        # I = input features
        # O = output features
        ...
        #query = query[:,-8:,:]
        #print(query.shape)
        #[batch_size/#TPU cores, 1, head_size]

        key = inputs[1]
        #print(key.shape)
        #[batch_size/#TPU cores, seq_length+1, head_size]
        ...
        # query NI -> NIH  (create H copies) 
        dimen_query_addhead = [1]*len(query.shape)+[self.num_heads]
        query = tf.expand_dims(query, -1)
        query = tf.tile(query, dimen_query_addhead)
        # swap the head to second last axis MIH -> MHI
        query = tf.einsum("...ij->...ji", query)


        #S= # query_features = I
        key = tf.einsum("...MI , HIS -> ...MHS", key, self.key_kernel_mod)
        value = tf.einsum("...MI , HIO -> ...MHO", value, self.value_kernel)
        ...
        if mask is not None:
            ...
            if len(mask.shape) != len(logits.shape):
                mask = tf.expand_dims(mask, -3)
            #(mask.shape [4,1,513,513] 
            # cut the size of mask to be compatible with query length
            #mask = mask[:,:,-8:,:]
        ...
        if attn_weight is not None:
            if len(attn_weight.shape) != len(logits.shape):
                attn_weight = tf.expand_dims(attn_weight, -3)
            # cut the size of attn_weight to be compatible with query length
...
def half_dense_layer(d_model, activation = 'relu'):
  actv = gelu if activation == 'gelu' else activation
  return tf.keras.Sequential([
      tf.keras.layers.Dense(d_model, activation=actv),  # (batch_size, seq_len, dff)
  ])
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        self.hocmha = HocMultiHeadAttention(
            model_dimension // attention_num_heads,
            attention_num_heads,
            dropout = attention_dropout,
            return_attn_coef = return_attn_coef,
        ...
        self.halfn = half_dense_layer(model_dimension, activation = 'relu')
    ...
    def call(self, inputs, training = None):
        ...
        attn1 = self.mha([value, value], 
                               mask=mask, 
                               attn_weight = attn_weight,
                               training = training)
        hocattn1 = attn1 #self.hocmha([value, value], mask=mask, attn_weight = attn_weight, training = training)
        if self.return_attn_coef:
            attn_output, attn_coef = attn1
        #value = value[:,-8:,:]
         #************************************************************
        # stack hocattention and attention together
        stack_out = tf.keras.layers.Concatenate(axis=-1)([hocattn1+ value, attn1 + value])
        #************************************************************
        out1 = self.halfn( self.layer_norm_1(stack_out))
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training = None):
        ...
        #************************************************************
        # stack hocattention and attention together
        stack_out = tf.keras.layers.Concatenate(axis=-1)([hocattn1+ query, attn1 + query])
        #************************************************************
        out1 = self.halfn( self.layer_norm_1(stack_out) )
        #print(out1.shape) 
        attn2 = self.mha_2([out1, encoder_out],
                           ...
                           training = training)
        hocattn2 = attn2 #self.hocmha([value, value], mask=mask, attn_weight = attn_weight, training = training)
        if self.return_attn_coef:
            attn2, eq_attn_coef = attn2
        #************************************************************
        # stack hocattention and attention together
        stack_out2 = tf.keras.layers.Concatenate(axis=-1)([hocattn2+ out1, attn2 + out1])
        #************************************************************
             
        out2 =  self.halfn( self.layer_norm_2(stack_out2) ) # (batch_size, target_seq_len, d_model)
        #print(out2.shape) 
